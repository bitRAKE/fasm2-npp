...
; Most of this file is automatically generated from the Scintilla.iface interface definition file which contains any comments about the definitions. HFacer.py does the generation.
...
;include "Sci_Position.g" ; These types are all qwords:
...
if ~ definite SCI_DISABLE_AUTOGENERATED
...
if ~ definite SCI_DISABLE_PROVISIONAL
...
end if ;~ definite SCI_DISABLE_PROVISIONAL
...
end if ;~ definite SCI_DISABLE_AUTOGENERATED
...
struct Sci_CharacterRange
	cpMin	dq ? ; Sci_PositionCR
	cpMax	dq ? ; Sci_PositionCR
ends

struct Sci_CharacterRangeFull
	cpMin	dq ? ; Sci_Position
	cpMax	dq ? ; Sci_Position
ends
...
struct Sci_TextToFindFull
	chrg		Sci_CharacterRangeFull
	lpstrText	dq ? ; const *char
	chrgText	Sci_CharacterRangeFull
ends
...
; This structure is used in printing and requires some of the graphics types from Platform.h.  Not needed by most client code.

struct Sci_RangeToFormat
	hdc		dq ?	; Sci_SurfaceID
	hdcTarget	dq ?	; Sci_SurfaceID
	rc		Sci_Rectangle
...
struct Sci_NotifyHeader
	...
	; hwndFrom is really an environment specific window handle or pointer but most clients of Scintilla.h do not have this type visible.
	hwndFrom	dq ?
	idFrom		dq ?
	code		dd ?,?
ends
...
struct SCNotification
	...
	_ch			dd ?
	; SCN_CHARADDED, SCN_KEY, SCN_AUTOCCOMPLETED
	; SCN_AUTOCSELECTION, SCN_USERLISTSELECTION
	modifiers		dd ?
	; SCN_KEY, SCN_DOUBLECLICK, SCN_HOTSPOTCLICK, SCN_HOTSPOTDOUBLECLICK
	; SCN_HOTSPOTRELEASECLICK, SCN_INDICATORCLICK, SCN_INDICATORRELEASE

	modificationType	dd ?	; SCN_MODIFIED
	__0			dd ?
	text			dq ?	; const char *
	; SCN_MODIFIED, SCN_USERLISTSELECTION, SCN_AUTOCSELECTION, SCN_URIDROPPED

	length			dq ?	; Sci_Position	; SCN_MODIFIED
	linesAdded		dq ?	; Sci_Position	; SCN_MODIFIED
	message			dd ?	; int		; SCN_MACRORECORD
	__1			dd ?
	wParam			dq ?	; uptr_t	; SCN_MACRORECORD
	lParam			dq ?	; sptr_t	; SCN_MACRORECORD
	line			dq ?	; Sci_Position	; SCN_MODIFIED
	foldLevelNow		dd ?	; int		; SCN_MODIFIED
	foldLevelPrev		dd ?	; int		; SCN_MODIFIED
	margin			dd ?	; int		; SCN_MARGINCLICK
	listType		dd ?	; int		; SCN_USERLISTSELECTION
	x			dd ?	; int		; SCN_DWELLSTART, SCN_DWELLEND
	y			dd ?	; int		; SCN_DWELLSTART, SCN_DWELLEND
	token			dd ?	; int		; SCN_MODIFIED with SC_MOD_CONTAINER
	__2			dd ?
	annotationLinesAdded	dq ?	; Sci_Position	; SCN_MODIFIED with SC_MOD_CHANGEANNOTATION
	updated			dd ?	; int		; SCN_UPDATEUI
	listCompletionMethod	dd ?	; int
	; SCN_AUTOCSELECTION, SCN_AUTOCCOMPLETED, SCN_USERLISTSELECTION
	characterSource		dd ?	; int		; SCN_CHARADDED
	__3			dd ?
ends

;#include <vector>
;struct SearchResultMarkingLine
; each line could have several segments if user want to see only 1 found line which contains several results
;	std::vector<std::pair<intptr_t, intptr_t>> _segmentPostions;
; a vector of pair of start & end of occurrence for colourizing
;ends

struct SearchResultMarkings
	_length		dq ?	; intptr_t
	_markings	dq ?	;*SearchResultMarkingLine
ends

if definite INCLUDE_DEPRECATED_FEATURES
...
end if ;definite INCLUDE_DEPRECATED_FEATURES
