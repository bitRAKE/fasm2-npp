; Note: *_DEPRECATED are not defined.
;------------------------------------------------------------------------------
...
iterate LangType,\
	L_TEXT,		L_PHP,		L_C,		L_CPP,\
	L_CS,		L_OBJC,		L_JAVA,		L_RC,\
	L_HTML,		L_XML,		L_MAKEFILE,	L_PASCAL,\
	L_BATCH,	L_INI,		L_ASCII,	L_USER,\
	L_ASP,		L_SQL,		L_VB,		L_JS,\
	L_CSS,		L_PERL,		L_PYTHON,	L_LUA,\
	L_TEX,		L_FORTRAN,	L_BASH,		L_FLASH,\
	L_NSIS,		L_TCL,		L_LISP,		L_SCHEME,\
	L_ASM,		L_DIFF,		L_PROPS,	L_PS,\
	L_RUBY,		L_SMALLTALK,	L_VHDL,		L_KIX,\
	L_AU3,		L_CAML,		L_ADA,		L_VERILOG,\
	L_MATLAB,	L_HASKELL,	L_INNO,		L_SEARCHRESULT,\
	L_CMAKE,	L_YAML,		L_COBOL,	L_GUI4CLI,\
	L_D,		L_POWERSHELL,	L_R,		L_JSP,\
	L_COFFEESCRIPT,	L_JSON,		L_JAVASCRIPT,	L_FORTRAN_77,\
	L_BAANC,	L_SREC,		L_IHEX,		L_TEHEX,\
	L_SWIFT,	L_ASN1,		L_AVS,		L_BLITZBASIC,\
	L_PUREBASIC,	L_FREEBASIC,	L_CSOUND,	L_ERLANG,\
	L_ESCRIPT,	L_FORTH,	L_LATEX,	L_MMIXAL,\
	L_NIM,		L_NNCRONTAB,	L_OSCRIPT,	L_REBOL,\
	L_REGISTRY,	L_RUST,		L_SPICE,	L_TXT2TAGS,\
	L_VISUALPROLOG,	L_TYPESCRIPT,	L_JSON5,	L_MSSQL,\
	L_GDSCRIPT,	L_HOLLYWOOD,	L_GOLANG,	L_RAKU,\
	L_TOML,		L_SAS,\
	L_EXTERNAL ; The end of enumerated language type, so it should be always at the end.
	match =L_JS,LangType
...
iterate winVer, WV_UNKNOWN,WV_WIN32S,WV_95,WV_98,WV_ME,WV_NT,WV_W2K,WV_XP,WV_S2003,WV_XPX64,WV_VISTA,WV_WIN7,WV_WIN8,WV_WIN81,WV_WIN10,WV_WIN11
	winVer := %-1
end iterate

iterate Platform, PF_UNKNOWN,PF_X86,PF_X64,PF_IA64,PF_ARM64
	Platform := %-1
end iterate

iterate _, Standard,C_Like,Custom
	ExternalLexerAutoIndentMode._ := %-1 ; enum class
end iterate

iterate _, Idle,RecordInProgress,RecordingStopped,PlayingBack
	MacroStatus._ := %-1 ; enum class
end iterate
...
NPPM_SAVESESSION := NPPMSG + 15
	struct sessionInfo
		sessionFilePathName	dq ? ; wchar_t* ; Full session file path name to be saved
		nbFile			dd ?,?		; Size of "files" array - number of files to be saved in session
		files			dq ?; wchar_t** ; Array of file name (full path) to be saved in session
	ends
...
;	NPPM_ADDTOOLBARICON_DEPRECATED := NPPMSG + 41
	struct toolbarIcons
		hToolbarBmp	dq ? ; HBITMAP
		hToolbarIcon	dq ? ; HICON
	ends
...
NPPM_MSGTOPLUGIN := NPPMSG + 47
	struct CommunicationInfo
		internalMsg	dd ?,?	; an integer defined by plugin Y, known by plugin X, identifying the message being sent.
		srcModuleName	dq ?	; const wchar_t*; the complete module name (with the extesion .dll) of caller (plugin X).
		info		dq ?	; void*; defined by plugin, the informations to be exchanged between X and Y. It's a void pointer so it should be defined by plugin Y and known by plugin X.
	ends
